...
{
    [ApiController]
    [Route("api/grok-3-mini")]
    [Produces("application/json")]
    public class Grok3MiniController : ControllerBase
    {
        private readonly HttpClient _httpClient;
        private readonly ILogger<Grok3MiniController> _logger;
        private readonly string _apiKey;
        private readonly string _baseUrl;

        public Grok3MiniController(IConfiguration configuration, IHttpClientFactory httpClientFactory, ILogger<Grok3MiniController> logger)
        {
            _logger = logger;
        ...
        [HttpPost("solve")]
        [SwaggerOperation(Summary = "Solve physics problem with Grok-3-mini API", Description = "Sends a physics problem to Grok-3-mini for detailed step-by-step solution. Uses streaming for faster response, lower temperature for accuracy.")]
        ...
        public async Task<IActionResult> Solve([FromBody] Gemini15ChatRequest request)
        {
            ...
            {
                _logger.LogWarning("Invalid request data for Grok-3-mini solve.");
...
Problem: {request.Message}";
            ...
            {
                _logger.LogError(ex, "Error processing Grok-3-mini API solve request.");
                return StatusCode(500, new { Message = "An error occurred while processing the request." });
            }
        }

        [HttpPost("latex")]
        [SwaggerOperation(Summary = "Get LaTeX code for physics problem solution with Grok-3-mini", Description = "Sends a physics problem to Grok-3-mini and returns only the LaTeX code for the solution.")]
        ...
        public async Task<IActionResult> Latex([FromBody] Gemini15ChatRequest request)
        {
            if (!ModelState.IsValid || string.IsNullOrWhiteSpace(request.Message))
            {
                _logger.LogWarning("Invalid request data for Grok-3-mini latex.");
                return BadRequest(new { Message = "Message is required." });
            }
...
Problem: {request.Message}";
            ...
            {
                _logger.LogError(ex, "Error processing Grok-3-mini API latex request.");
                return StatusCode(500, new { Message = "An error occurred while processing the request." });
            }
        }

        [HttpPost("chat")]
        [SwaggerOperation(Summary = "General chat with Grok-3-mini API", Description = "Sends a message to Grok-3-mini for general response.")]
        ...
        public async Task<IActionResult> Chat([FromBody] Gemini15ChatRequest request)
        {
            if (!ModelState.IsValid || string.IsNullOrWhiteSpace(request.Message))
            {
                _logger.LogWarning("Invalid request data for Grok-3-mini chat.");
                return BadRequest(new { Message = "Message is required." });
            }
            ...
            {
                _logger.LogError(ex, "Error processing Grok-3-mini API chat request.");
                return StatusCode(500, new { Message = "An error occurred while processing the request." });
            }
        }
    }
